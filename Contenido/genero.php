...
if(isset($_REQUEST['action']))
{
	switch($_REQUEST['action'])
	{
		case 'actualizar':
			...
			$alm->__SET('desc_gen',         $_REQUEST['desc_gen']);
		...
		case 'eliminar':
			$model->Eliminar($_REQUEST['idgenero']);
			header('Location: genero.php');
			break;

		case 'editar':
			$alm = $model->Obtener($_REQUEST['idgenero']);
			break;
	}
}
    ...
    <body style="padding:15px;">
        ...
        <div class="pure-g">
            <div class="pure-u-1-12">
              <div align="center">
                <form action="?action=<?php echo $alm->idgenero > 0 ? 'actualizar' : 'registrar'; ?>" method="post" class="pure-form pure-form-stacked" style="margin-bottom:30px;">
                    <input type="hidden" name="idgenero" value="<?php echo $alm->__GET('idgenero'); ?>" />
                    
                      <table width="87%" border="0">
                      
                        <tr>
                            <th width="112" style="text-align:left;">Tipo Respuesta</th>
                            <td width="500"><input type="text" name="desc_gen" placeholder="Tipo de genero" required value="<?php echo $alm->__GET('desc_gen'); ?>" style="width:100%;" /></td>
                        </tr>
                       </table>
                ...
                <table class="pure-table pure-table-horizontal">
                    ...
                    <?php foreach($model->Listar() as $r): ?>
                        <tr>
                            ...
                            <td><?php echo $r->__GET('desc_gen'); ?></td>
                            <td>&nbsp;</td>
                            <td>
                                <a class="w3-btn w3-round-xxlarge"href="?action=editar&idgenero=<?php echo $r->idgenero; ?>">Editar</a> <a class="w3-btn w3-round-xxlarge" href="?action=eliminar&idgenero=<?php echo $r->idgenero; ?>">Eliminar</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </table>     
